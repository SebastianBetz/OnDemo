...
contract Election {
    ...
    mapping(address => uint) private candidateToOption;
    ...
    modifier isActive() {
        bool active = false;
        if(state == State.CREATED || state == State.APPROVED || state == State.STARTED)
        {
    ...
    function startConsultation(address[] memory _leaders) public {
        if(state == State.APPROVED)
        {
            ...
            {
                setState(State.STARTED, "Council has approved the referendum!");
            }
            else{
                revert("Only Members of the council can approve consultations.");
            }
    ...
    function finishConsultation() private {
        if(state == State.STARTED)
        {
            ...
            revert("Consultation is not in the 'Started' state and can therefore not be finished.");
        }           
    }
