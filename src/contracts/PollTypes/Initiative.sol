...
contract Initiative {
    // A initiative contract that uses the polling interface
    // A initiative can have multiple answers that are created by members. It has a deadline and can be started by any member.
    // A initiative has 3 phases.
    // 1.Phase: Members can create answers and express their support for the initiative. A min amount of support needs to be gathered before the deadline to continue.
    // 2.Phase: Members can create answers and express their support for the initiative. A min amount of support needs to be gathered before the deadline to continue.
    // 3.Phase: The Council approves or cancels the initiative. If approved the members can vote on ONE answer (Exclusive Voting).
    // A initiative extends the polling interface to include role based rights inherited from the accountmanagement contract.
    ...
    enum State { 
        CREATED, 
        STAGE1,             // Initiative has gathered enough support before the first deadline
        STAGE2,             // Initiative has gathered enough support before the second deadline
        APPROVEDBYCOUNCIL,  // Initiative has been approved by council
        PUBLISHED,          // Initiative has been published by council
        CLOSED,             
        CANCELED 
    } 
    ...
    function support() onlyIfIsActive public {
        // initiatives need support to push through stage 1 + 2. Here users can add their support
        if(canSupport()){
    ...
    function removeSupport() onlyIfIsActive public {
        // removes support from the initiative
    ...
    function close() onlyCouncelors public {
        setState(State.CLOSED, "council has closed the initiative");
    }
